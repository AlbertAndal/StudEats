    ...
    <div class="max-w-md w-full space-y-8">
        ...
        <div>
            ...
            <p class="mt-2 text-center text-sm text-gray-600">
                ...
                <span class="font-medium text-green-600"><?php echo e($user->email); ?></span>
            </p>
        </div>
        ...
        <?php if(config('app.env') === 'local' && isset($latestOtp) && $latestOtp): ?>
                ...
                <div class="flex">
                    ...
                    <div class="ml-3">
                        ...
                        <div class="mt-1 text-sm text-yellow-700">
                            <p>Your OTP code is <strong><?php echo e($latestOtp->otp_code); ?></strong></p>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div id="troubleshooting-panel" class="hidden bg-gray-50 border border-gray-200 rounded px-4 py-3">
            ...
            <ul class="space-y-1 text-sm text-gray-600">
                ...
                <li class="flex items-start">
                    <span class="inline-block w-1 h-1 bg-gray-400 rounded-full mt-2 mr-2 flex-shrink-0"></span>
                    <span>Verify that <strong><?php echo e($user->email); ?></strong> is the correct email</span>
                </li>
            </ul>
            <div class="mt-3 text-xs text-gray-500">
                Still having issues? 
                <a href="<?php echo e(route('register')); ?>" class="text-green-600 hover:text-green-500">Start over with registration</a> or 
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    otpForm.addEventListener('submit', async function(e) {
        ...
        try {
            const response = await fetch('<?php echo e(route("email-verification.verify-otp")); ?>', {
                method: 'POST',
                headers: {
            ...
            if (data.success) {
                ...
                setTimeout(async () => {
                    try {
                        const completeResponse = await fetch('<?php echo e(route("email-verification.complete")); ?>', {
                            method: 'POST',
                            headers: {
                                'Content-Type': 'application/json',
                                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
    ...
    resendBtn.addEventListener('click', async function() {
        ...
        try {
            const response = await fetch('<?php echo e(route("email-verification.resend-otp")); ?>', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\StudEats\resources\views/auth/verify-email.blade.php ENDPATH**/ ?>
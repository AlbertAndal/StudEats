    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        <div class="bg-white shadow rounded-lg p-6">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        <p class="text-2xl font-bold text-gray-900">{{ $recipes->total() }}</p>
        ...
        <div class="bg-white shadow rounded-lg p-6">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        <p class="text-2xl font-bold text-gray-900">{{ $recipes->where('is_featured', true)->count() }}</p>
        ...
        <div class="bg-white shadow rounded-lg p-6">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        <p class="text-2xl font-bold text-gray-900">{{ $cuisineTypes->count() }}</p>
        ...
        <div class="bg-white shadow rounded-lg p-6">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        <p class="text-2xl font-bold text-gray-900">{{ number_format($recipes->sum('meal_plans_count')) }}</p>
        ...
        <div class="px-6 py-4">
            <form method="GET" action="{{ route('admin.recipes.index') }}" class="space-y-4 sm:space-y-0 sm:flex sm:items-end sm:gap-4">
                ...
                <div class="sm:w-48">
                    ...
                    <select name="cuisine_type" class="w-full py-3 px-4 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                        ...
                        @foreach($cuisineTypes as $cuisine)
                            <option value="{{ $cuisine }}" {{ request('cuisine_type') === $cuisine ? 'selected' : '' }}>
                                {{ ucfirst($cuisine) }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="flex gap-2">
                    ...
                    @if(request()->hasAny(['search', 'cuisine_type', 'difficulty']))
    ...
    <div class="bg-white shadow rounded-lg overflow-hidden">
                ...
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    <h3 class="text-lg font-semibold text-gray-900">
                        ...
                        <span class="text-sm font-normal text-gray-500 ml-2">
                            ({{ $recipes->firstItem() ?? 0 }}-{{ $recipes->lastItem() ?? 0 }} of {{ $recipes->total() }})
                        </span>
                    </h3>
            ...
            <div class="overflow-x-auto">
                <table class="w-full">
                    ...
                    <tbody class="divide-y divide-gray-200">
                        @forelse($recipes as $recipe)
                            <tr class="hover:bg-gray-50 transition-colors">
                                <td class="px-6 py-4">
                                    <div class="flex items-center">
                                        @if($recipe->image_url)
                                            ...
                                            <div class="w-16 h-16 bg-gradient-to-br from-orange-400 to-pink-500 rounded-lg flex items-center justify-center text-white font-bold text-lg mr-4" style="display:none;" title="Image failed to load">
                                ...
                                <td class="px-6 py-4">
                                    <div class="space-y-1">
                                        <div class="text-sm font-medium text-gray-900">{{ number_format($recipe->meal_plans_count) }} plans</div>
                                        <div class="text-sm text-gray-500">Added {{ $recipe->created_at->diffForHumans() }}</div>
                                    </div>
                                </td>
                                <td class="px-6 py-4">
                                    <div class="space-y-2">
                    ...
                    <button data-delete-recipe="{{ $recipe->id }}" data-recipe-name="{{ e($recipe->name) }}" 
                            ...
                            <tr>
                                <td colspan="5" class="px-6 py-12 text-center">
                                    <div class="flex flex-col items-center">
                                        ...
                                        <p class="text-gray-500 mb-6">{{ request()->hasAny(['search', 'cuisine_type', 'difficulty']) ? 'Try adjusting your search filters.' : 'Get started by creating your first recipe.' }}</p>
                                        @if(!request()->hasAny(['search', 'cuisine_type', 'difficulty']))
                                            <a href="{{ route('admin.recipes.create') }}" 

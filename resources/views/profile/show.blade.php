                    ...
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-sm text-orange-700 mt-1">
                                Your email address <strong>{{ $user->email }}</strong> needs to be verified to access all features and ensure account security.
                            </p>
                        </div>
                        <div class="flex items-center space-x-3 ml-4">
                            <form method="POST" action="{{ route('email.verify.resend') }}" class="inline">
                                @csrf
            ...
            <div class="flex items-center">
                ...
                <div class="ml-3">
                    <p class="text-sm font-medium text-green-800">
                        ...
                        <span class="text-green-600 ml-2">Verified on {{ $user->email_verified_at->format('M d, Y \a\t h:i A') }}</span>
                    </p>
                </div>
            </div>
        </div>
        ...
        <div class="mt-6 space-y-6">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div class="space-y-4">
                    ...
                    <!-- Dietary Preferences -->
                    <section aria-labelledby="dietary-preferences-title">
                        <div class="bg-white shadow-sm border border-gray-200 rounded-lg overflow-hidden hover:shadow-md transition-shadow duration-200">
                            <div class="px-4 py-3 bg-gradient-to-r from-orange-50/50 to-white border-b border-gray-200">
                                <div class="flex items-center gap-2">
                                    <div class="flex-shrink-0">
                                        <div class="h-8 w-8 bg-orange-500/10 rounded-lg flex items-center justify-center">
                                    ...
                                    <div class="flex-1">
                                        <h2 id="dietary-preferences-title" class="text-base font-semibold text-gray-900 leading-5">Diet Preferences</h2>
                                        <p class="text-xs text-gray-500">Food preferences and restrictions</p>
                                    </div>
                                </div>
                            </div>
                            <div class="px-4 py-3">
                                @if($user->dietary_preferences && count($user->dietary_preferences) > 0)
                                    @php
                                        ...
                                        $preferenceConfig = [
                                            // Diet Types
                                            'vegetarian' => ['color' => 'bg-green-100 text-green-800 border-green-200', 'icon' => '🥬', 'description' => 'No meat, fish, or poultry', 'category' => 'Diet Types'],
                                            'vegan' => ['color' => 'bg-emerald-100 text-emerald-800 border-emerald-200', 'icon' => '🌱', 'description' => 'No animal products', 'category' => 'Diet Types'],
                                            'pescatarian' => ['color' => 'bg-teal-100 text-teal-800 border-teal-200', 'icon' => '🐟', 'description' => 'Fish but no meat', 'category' => 'Diet Types'],
                                            'keto' => ['color' => 'bg-indigo-100 text-indigo-800 border-indigo-200', 'icon' => '�', 'description' => 'Very low carb, high fat', 'category' => 'Diet Types'],
                                            'paleo' => ['color' => 'bg-amber-100 text-amber-800 border-amber-200', 'icon' => '🥩', 'description' => 'Whole foods, no processed', 'category' => 'Diet Types'],
                                            'mediterranean' => ['color' => 'bg-cyan-100 text-cyan-800 border-cyan-200', 'icon' => '🫒', 'description' => 'Heart-healthy, olive oil based', 'category' => 'Diet Types'],
                                            
                                            // Food Restrictions
                                            'gluten_free' => ['color' => 'bg-yellow-100 text-yellow-800 border-yellow-200', 'icon' => '🌾', 'description' => 'No wheat, barley, rye', 'category' => 'Restrictions'],
                                            'dairy_free' => ['color' => 'bg-blue-100 text-blue-800 border-blue-200', 'icon' => '🥛', 'description' => 'No milk products', 'category' => 'Restrictions'],
                                            'nut_free' => ['color' => 'bg-orange-100 text-orange-800 border-orange-200', 'icon' => '🥜', 'description' => 'No tree nuts or peanuts', 'category' => 'Restrictions'],
                                            'shellfish_free' => ['color' => 'bg-red-100 text-red-800 border-red-200', 'icon' => '🦐', 'description' => 'No shellfish', 'category' => 'Restrictions'],
                                            'soy_free' => ['color' => 'bg-lime-100 text-lime-800 border-lime-200', 'icon' => '🫘', 'description' => 'No soy products', 'category' => 'Restrictions'],
                                            'egg_free' => ['color' => 'bg-rose-100 text-rose-800 border-rose-200', 'icon' => '🥚', 'description' => 'No eggs or egg products', 'category' => 'Restrictions'],
                                            
                                            // Nutritional Goals
                                            'low_carb' => ['color' => 'bg-purple-100 text-purple-800 border-purple-200', 'icon' => '⚡', 'description' => 'Reduced carbohydrates', 'category' => 'Goals'],
                                            'high_protein' => ['color' => 'bg-pink-100 text-pink-800 border-pink-200', 'icon' => '💪', 'description' => 'Extra protein for fitness', 'category' => 'Goals'],
                                            'low_sodium' => ['color' => 'bg-slate-100 text-slate-800 border-slate-200', 'icon' => '🧂', 'description' => 'Reduced salt intake', 'category' => 'Goals'],
                                            'heart_healthy' => ['color' => 'bg-red-100 text-red-800 border-red-200', 'icon' => '❤️', 'description' => 'Good for cardiovascular health', 'category' => 'Goals'],
                                            'diabetic_friendly' => ['color' => 'bg-violet-100 text-violet-800 border-violet-200', 'icon' => '🩺', 'description' => 'Low glycemic index', 'category' => 'Goals'],
                                            'weight_loss' => ['color' => 'bg-sky-100 text-sky-800 border-sky-200', 'icon' => '📉', 'description' => 'Calorie-controlled portions', 'category' => 'Goals'],
                                        ];
                                        
                                        // Group preferences by category
                                        $groupedPreferences = [];
                                        foreach ($user->dietary_preferences as $preference) {
                                            $config = $preferenceConfig[$preference] ?? [
                                                'color' => 'bg-gray-100 text-gray-800 border-gray-200',
                                                'icon' => '🍽️',
                                                'description' => 'Diet preference',
                                                'category' => 'Other'
                                            ];
                                            $groupedPreferences[$config['category']][] = [
                                                'key' => $preference,
                                                'config' => $config
                                            ];
                                        }
                                    ...
                                    <div class="space-y-4">
                                        @foreach($groupedPreferences as $category => $preferences)
                                            <div class="space-y-2">
                                                <h4 class="text-xs font-semibold text-gray-600 uppercase tracking-wider">{{ $category }}</h4>
                                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-2">
                                                    @foreach($preferences as $pref)
                                                        <div class="group relative">
                                                            <div class="flex items-center px-3 py-2 rounded-lg text-xs font-medium border {{ $pref['config']['color'] }} transition-all duration-200 hover:shadow-sm cursor-default w-full">
                                                                <span class="text-sm mr-2 flex-shrink-0">{{ $pref['config']['icon'] }}</span>
                                                                <div class="flex-1 min-w-0">
                                                                    <span class="font-semibold">{{ ucfirst(str_replace('_', ' ', $pref['key'])) }}</span>
                                                                    <div class="text-xs opacity-75 truncate">{{ $pref['config']['description'] }}</div>
                                                                </div>
                                                            </div>
                                                            <!-- Enhanced Tooltip -->
                                                            <div class="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 px-3 py-2 text-xs text-white bg-gray-900 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none whitespace-nowrap z-20 shadow-lg">
                                                                <div class="font-medium">{{ ucfirst(str_replace('_', ' ', $pref['key'])) }}</div>
                                                                <div class="text-gray-300">{{ $pref['config']['description'] }}</div>
                                        ...
                                        <div class="bg-gradient-to-r from-orange-50 via-yellow-50 to-orange-50 rounded-xl p-4 border border-orange-200 mt-6">
                                            <div class="flex items-start gap-3">
                                                <div class="flex-shrink-0 w-10 h-10 bg-orange-100 rounded-full flex items-center justify-center">
                                                    <span class="text-lg">💡</span>
                                                </div>
                                                <div class="flex-1">
                                                    <h4 class="text-sm font-semibold text-orange-800 mb-2">Personalized Meal System</h4>
                                                    <div class="space-y-2 text-xs text-orange-700">
                                                        <div class="flex items-center gap-2">
                                                            <span class="w-1.5 h-1.5 bg-orange-400 rounded-full flex-shrink-0"></span>
                                                            <span>{{ count($user->dietary_preferences) }} dietary preference{{ count($user->dietary_preferences) !== 1 ? 's' : '' }} active</span>
                                                        </div>
                                                        <div class="flex items-center gap-2">
                                                            <span class="w-1.5 h-1.5 bg-orange-400 rounded-full flex-shrink-0"></span>
                                                            <span>Meals filtered to match your dietary needs</span>
                                                        </div>
                                                        <div class="flex items-center gap-2">
                                                            <span class="w-1.5 h-1.5 bg-orange-400 rounded-full flex-shrink-0"></span>
                                                            <span>Nutritional recommendations personalized for you</span>
                                                        </div>
                                                        @if($user->daily_budget)
                                                            <div class="flex items-center gap-2">
                                                                <span class="w-1.5 h-1.5 bg-orange-400 rounded-full flex-shrink-0"></span>
                                                                <span>Budget-friendly options within ₱{{ number_format($user->daily_budget, 0) }} daily limit</span>
                                                            </div>
                                                        @endif
                                                    </div>
                                                    <div class="mt-3 pt-3 border-t border-orange-200">
                                                        <a href="{{ route('profile.edit') }}" 
                                                           class="inline-flex items-center text-xs font-medium text-orange-700 hover:text-orange-800 transition-colors">
                                ...
                                @else
                                    <div class="text-center py-4">
                                        ...
                                        <h3 class="text-sm font-medium text-gray-900 mb-1">No Dietary Preferences</h3>
                                        <p class="text-gray-500 mb-3 text-xs">Add preferences for personalized recommendations</p>
                                        <a href="{{ route('profile.edit') }}" 
                                           class="inline-flex items-center px-3 py-1.5 text-xs font-medium rounded text-white bg-orange-600 hover:bg-orange-700 transition-colors">
                                            + Add Preferences
                                        </a>
                                    </div>
                                @endif
                    ...
                    <div class="bg-white shadow-sm border border-gray-200 rounded-lg overflow-hidden hover:shadow-md transition-shadow duration-200">
                        ...
                        <div class="bg-gradient-to-r from-gray-50 to-white px-8 py-6 border-t border-gray-100">
                            ...
                            <div class="flex items-center justify-between mb-2">
                                ...
                                <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-bold {{ $scoreBg }} {{ $scoreColor }}">
                                    {{ $securityScore }}%
                                </span>
                            </div>
                            ...
                            <div class="w-full bg-gray-200 rounded-full h-1.5 mb-2">
                                <div class="h-1.5 rounded-full transition-all duration-300 {{ $securityScore >= 80 ? 'bg-green-500' : ($securityScore >= 60 ? 'bg-yellow-500' : 'bg-red-500') }}" 
